...
from app.models import Project, User, Payment, Delivery, Dispute, Withdrawal
    ...
    for admin_id in settings.admin_ids:
        try:
            await bot.send_message(
                admin_id,
                text,
                reply_markup=admin_project(project_id) if project_id else None,
            )
        except Exception:
            pass
...
async def choose_role(call: CallbackQuery):
    ...
    username = (call.from_user.username or "").lstrip("@")
    ...
    if role == "seller":
        await call.message.answer("Seller account activated.", reply_markup=seller_menu())
    else:
        await call.message.answer("Client account activated.", reply_markup=client_menu())
...
async def admin(message: Message):
    ...
    text = "Admin Dashboard\n\n" + "\n".join(
        [f"#{p.id} - {p.status} - ${p.budget} - {p.project_type}" for p in projects]
    )
...
async def seller_accept(call: CallbackQuery, bot: Bot):
    ...
    async with AsyncSessionLocal() as session:
        ...
        project = await session.get(Project, pid)
        if not project:
            return await call.message.answer("Project not found.")
    ...
    await call.message.answer("Offer accepted. Waiting for client payment.", reply_markup=seller_menu())
...
async def coin_select(call: CallbackQuery, bot: Bot):
    ...
    wallets = {
        ...
        "USDC": settings.usdc_wallet_address,
        "BTC": settings.btc_wallet_address,
    }
    ...
    await call.message.answer_photo(
        BufferedInputFile(buf.read(), filename="payment_qr.png"),
        caption=(
            f"Project #{pid}\n"
            f"Pay {payment.amount_due:.2f} USD worth of {coin}\n"
            f"Wallet:\n`{wallet}`\n\n"
            f"After payment, send your transaction hash here."
        ),
        parse_mode="Markdown",
    )
...
async def text_state(message: Message, bot: Bot):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if action == "tx_hash":
            ...
            await notify_admins(
                bot,
                f"Project #{pid} TX hash submitted:\n{message.text}\nConfirm after checking payment.",
                pid,
            )
            ...
            if client and client.telegram_id:
                await bot.send_message(
                    client.telegram_id,
                    f"Seller submitted project #{pid}:\n{message.text}",
                    reply_markup=client_review(pid),
                )
            ...
            await notify_admins(
                bot,
                f"Withdrawal request: @{user.telegram_username} ${amount}\nWallet: {message.text}",
            )
...
async def admin_confirm(call: CallbackQuery, bot: Bot):
    if not is_admin(call.from_user.id):
        return await call.answer("Admin only")
    ...
    async with AsyncSessionLocal() as session:
        ...
        project.deadline_at = datetime.utcnow() + timedelta(days=project.timeline_days)
    ...
    if seller and seller.telegram_id:
        await bot.send_message(
            seller.telegram_id,
            f"Project #{pid}: Payment confirmed. Start working.",
            reply_markup=seller_project(pid),
        )

    if client and client.telegram_id:
        await bot.send_message(
            client.telegram_id,
            f"Project #{pid}: Payment confirmed. Project is now in progress.",
        )
...
async def approve(call: CallbackQuery, bot: Bot):
    ...
    await call.message.answer("Project approved. Funds released to seller balance.")

    if seller and seller.telegram_id:
        await bot.send_message(
            seller.telegram_id,
            f"Project #{pid} approved. Your balance has been updated.",
        )
...
async def revision(call: CallbackQuery, bot: Bot):
    ...
    if seller and seller.telegram_id:
        await bot.send_message(
            seller.telegram_id,
            f"Revision requested for project #{pid}.",
            reply_markup=seller_project(pid),
        )
...
async def cancel(call: CallbackQuery, bot: Bot):
    ...
    if seller and seller.telegram_id:
        await bot.send_message(
            seller.telegram_id,
            f"Client requested cancellation for project #{pid}. Admin handles final split if needed.",
        )
...
async def admin_extend(call: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        project = await session.get(Project, pid)
        if project:
            project.deadline_at = (project.deadline_at or datetime.utcnow()) + timedelta(days=2)
...
async def withdraw_button(call: CallbackQuery):
    STATE[call.from_user.id] = ("withdraw_amount", None)
